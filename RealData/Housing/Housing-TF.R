...
# Preprocess the data: Remove rows with missing values in key columns
data <- data %>% filter(!is.na(latitude) & !is.na(longitude) & !is.na(median_house_value))

# Binning the data into a 100x100 grid based on latitude and longitude
lat_bins <- cut(data$latitude, breaks = 200, labels = FALSE)
lon_bins <- cut(data$longitude, breaks = 200, labels = FALSE)

# Create grid index and compute median house value per cell
data$grid_cell <- paste(lat_bins, lon_bins, sep = "_")
house_value <- data %>%
  group_by(grid_cell) %>%
  summarize(median_value = median(median_house_value))

# Take the log of house values
house_value$median_value <- log(house_value$median_value + 1)

# Extract counts as response variable y
y <- house_value$median_value
n <- length(y)

# Set evaluation points Lambda
Lambda_1 <- seq(5, 15, length.out = 100)
...
#write.csv(crps_trend_results, file.path(output_dir, "housing_trend_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_trend_results, file.path(output_dir, "housing_trend_max_sq.csv"), row.names = FALSE)
#write.csv(crps_spline_results, file.path(output_dir, "housing_spline_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_spline_results, file.path(output_dir, "housing_spline_max_sq.csv"), row.names = FALSE)
