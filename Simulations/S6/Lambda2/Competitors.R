...
set.seed(123)
n <- 1600
Lambda_1 <- seq(-2, 2, length.out = 100) # Evaluation points
num_reps <- 100
train_ratio <- 0.75

# Create output directory for results
output_dir <- "/Users/carlos/Desktop/Scenario 6/Lambda2"
...
for (rep in 1:num_reps) {
  ...
  drf_model <- drf(X = x_train, Y = matrix(y_train, ncol = 1), num.trees = 100, splitting.rule = "FourierMMD")
  ...
  Yhat_samples <- predict(engr_model, x_test, type = "sample", nsample = 300)
...
#write.csv(crps_cart_results, file.path(output_dir, "S6_n_1600_Lambda_2_cart_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_cart_results, file.path(output_dir, "S6_n_1600_Lambda_2_cart_max_sq.csv"), row.names = FALSE)
#write.csv(crps_mars_results, file.path(output_dir, "S6_n_1600_Lambda_2_mars_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_mars_results, file.path(output_dir, "S6_n_1600_Lambda_2_mars_max_sq.csv"), row.names = FALSE)
#write.csv(crps_rf_results, file.path(output_dir, "S6_n_1600_Lambda_2_rf_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_rf_results, file.path(output_dir, "S6_n_1600_Lambda_2_rf_max_sq.csv"), row.names = FALSE)
#write.csv(crps_drf_results, file.path(output_dir, "S6_n_1600_Lambda_2_drf_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_drf_results, file.path(output_dir, "S6_n_1600_Lambda_2_drf_max_sq.csv"), row.names = FALSE)
#write.csv(crps_engression_results, file.path(output_dir, "S6_n_1600_Lambda_2_engression_crps.csv"), row.names = FALSE)
#write.csv(max_sq_diff_engression_results, file.path(output_dir, "S6_n_1600_Lambda_2_engression_max_sq.csv"), row.names = FALSE)
