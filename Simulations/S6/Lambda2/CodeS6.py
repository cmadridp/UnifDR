...
# Parameters
n = 1600
Lambda_1 = np.linspace(-2, 2, 100)  # Evaluation points
num_reps = 100
input_dim = 10
hidden_dim = 64
output_dim = 1
...
# Save results to CSV files
pd.DataFrame(crps_dnn_results, columns=["CRPS"]).to_csv(os.path.join(output_dir, "S6_n_1600_Lambda_2_dnn_crps.csv"), index=False)
pd.DataFrame(max_sq_diff_dnn_results, columns=["Max_Squared_Difference"]).to_csv(os.path.join(output_dir, "S6_n_1600_Lambda_2_dnn_max_sq.csv"), index=False)
