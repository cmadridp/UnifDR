...
for (rep in 1:num_reps) {
  ...
  F_hat_test_trend <- sapply(Lambda_1, function(t) {
    ...
    trend_model <- glmgen::trendfilter(x = w_t, y = NULL, k = k,lambda = exp(10))
    # Predict using test indices
    predict(trend_model, x.new = test_idx, lambda = exp(10))
  })
  F_hat_test_spline <- sapply(Lambda_1, function(t) {
    ...
    spline_model <- smooth.spline( w_t, spar = 1.01) # Fit smoothing spline
...
# Save results to CSV files
write.csv(crps_trend_results, "S3_n_400_Lambda_2_trend_crps.csv", row.names = FALSE)
write.csv(max_sq_diff_trend_results, "S3_n_400_Lambda_2_trend_max_sq.csv", row.names = FALSE)
write.csv(crps_spline_results, "S3_n_400_Lambda_2_spline_crps.csv", row.names = FALSE)
write.csv(max_sq_diff_spline_results, "S3_n_400_Lambda_2_spline_max_sq.csv", row.names = FALSE)
