...
for (rep in 1:num_reps) {
  # Generate data
  mu <- c(rep(6, n / 4), rep(2, n / 4), rep(8, n / 4), rep(4, n / 4))
  y <- rgamma(n, shape = 0.7, scale = mu) # Gamma(shape=2, scale=mu/2)
  ...
  F_hat_test_trend <- sapply(Lambda_1, function(t) {
    ...
    trend_model <- glmgen::trendfilter(x = w_t, y = NULL, k = k,lambda = 55)
    # Predict using test indices
    predict(trend_model, x.new = test_idx, lambda = 55)
  })
  F_hat_test_spline <- sapply(Lambda_1, function(t) {
    ...
    spline_model <- smooth.spline( w_t, spar = 1.05) # Fit smoothing spline
...
output_dir <- "/Users/carlos/Desktop/Scenario 3/Lambda2"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE) # Create the directory if it doesn't exist
}
write.csv(crps_trend_results, file.path(output_dir, "S4_n_1600_Lambda_2_trend_crps.csv"), row.names = FALSE)
write.csv(max_sq_diff_trend_results, file.path(output_dir, "S4_n_1600_Lambda_2_trend_max_sq.csv"), row.names = FALSE)
write.csv(crps_spline_results, file.path(output_dir, "S4_n_1600_Lambda_2_spline_crps.csv"), row.names = FALSE)
write.csv(max_sq_diff_spline_results, file.path(output_dir, "S4_n_1600_Lambda_2_spline_max_sq.csv"), row.names = FALSE)
